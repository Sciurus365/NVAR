...
#'
#' @return An `NVAR` object that contains `data`, a tidy form of `tibble` that contains the training data, and `W_out`, the parameter fit result.
#'
#' @export
...
NVAR <- function(data, vars, s, k, p, constant = TRUE, alpha = 0.05) {
  ...
  td <- make_tidy_data(data, vars, s, k, p, constant)
  Y <- as.matrix(td[,attr(td, "vars")])
  O_total <- as.matrix(td[,attr(td, "features")])
  W_out = NULL
  W_out <- t(Y) %*% O_total %*% (t(O_total) %*% O_total + alpha * diag(ncol(O_total)))

  return(structure(list(data = td, W_out = W_out), class = "NVAR"))
}

make_tidy_data <- function(data, vars, s, k, p, constant) {
  ...
  df <- df %>%
    ...
    dplyr::bind_cols(data[training_time[1:(length(training_time)-1)],vars])
  if(constant) {
    df$`1` <- 1
